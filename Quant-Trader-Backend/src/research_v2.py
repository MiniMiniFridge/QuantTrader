...
def get_trade_files(directory: str, sym: str) -> List[Path]:
    """
    Get all files in directory that match stock data patterns.
    For yfinance compatibility, this can be used to cache downloaded data.
    
    Args:
        directory: Path to directory to search
        sym: Stock symbol/ticker (e.g., 'AAPL', 'MSFT')
    
    Returns:
        List of Path objects matching stock patterns
    
    Example:
        >>> files = get_trade_files('./data', 'AAPL')
        >>> # Returns: ['AAPL_2024-09-22.parquet', 'AAPL_trades_2024.csv', ...]
    """
    dir_path = Path(directory)
    patterns = [
        f"{sym}_*.parquet",
        f"{sym}_*.csv",
        f"{sym}-*.parquet",  # backward compatibility
        f"{sym}-*.csv",
    ]
...
def download_ohlc_from_yfinance(
    sym: str,
    interval: str,
    start_date: datetime,
    end_date: datetime,
    ...
    """
    Download OHLC data from yfinance and convert to our standard format.
    """
    try:
    ...
    if data.empty:
        if show_progress:
            tqdm.write(f"❌ No data returned for {sym}")
        raise ValueError(f"No data returned from yfinance for {sym}")
    
    # Convert to Polars DataFrame and standardize column names
    df = pl.from_pandas(data.reset_index())
    
    # FIX: Use 'Datetime' instead of 'Date' in the mapping
    column_mapping = {
        'Datetime': 'datetime',
        'Open': 'open',
        'High': 'high', 
    ...
    df = df.rename({col: column_mapping.get(col, col) for col in df.columns})
    
    # Ensure datetime column exists and is correct type
    if "datetime" not in df.columns:
        raise ValueError("Datetime column not found in downloaded data")
    
    df = df.with_columns(pl.col("datetime").cast(pl.Datetime))
    
    # Cache the data if path provided
    if data_path:
        cache_dir = Path(data_path)
        cache_dir.mkdir(exist_ok=True)
        cache_file = cache_dir / f"{sym}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.parquet"
        df.write_parquet(cache_file)
        if show_progress:
            tqdm.write(f" Cached {sym} to: {cache_file}")
    
    if show_progress:
        tqdm.write(f" Downloaded {sym}: {len(df)} rows")
...
def load_timeseries(
    ...
    """
    Load stock time series data. For yfinance, we'll create a wrapper
    that either loads cached data or downloads fresh data.
    
    Note: yfinance provides OHLCV data directly, so aggregation is different.
    """
    if data_path is None:
        data_path = './cache'
    
    # First try to load from cached files
    files = get_trade_files(data_path, sym)
    
    if files:
        # Load from existing cached data
        ts_list = []
        for file in tqdm(files, desc=f"Loading cached {sym}", unit="file"):
            try:
                ...
                if "datetime" not in data.columns and "Date" in data.columns:
                    data = data.rename({"Date": "datetime"})
                
                data = data.with_columns(pl.col("datetime").cast(pl.Datetime))
                ts_list.append(data)
    ...
    # If no cached data, we'll need to download from yfinance
    tqdm.write(f"[INFO] No cached data found for {sym}. Consider using download_ohlc_from_yfinance()")
    raise FileNotFoundError(f"No cached data found for {sym} in {data_path}")
...
def load_timeseries_range(
    ...
    """
    Load stock time series data for a date range.
    For yfinance integration, this would download data directly.
    """
    if data_path is None:
        data_path = "./cache"
    ...
    cache_file = Path(data_path) / f"{sym}_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.parquet"
    ...
    if cache_file.exists():
        try:
            ...
            tqdm.write(f"[ERROR] Loading cached data: {e}")

    # If no cache, suggest using yfinance directly
    tqdm.write(f"[INFO] No cached data found for {sym}. Use download_ohlc_from_yfinance()")
    raise FileNotFoundError(f"No cached data found for {sym} in range {start_date} to {end_date}")
...
def load_ohlc_timeseries_range(
    ...
    """
    Load and aggregate stock data for a symbol between start_date and end_date
    into OHLC time series using the given time interval.

    Args:
        sym: Stock symbol (e.g., 'AAPL', 'MSFT')
        time_interval: Aggregation interval (e.g., '1h', '5m')
        start_date: Start datetime (inclusive)
        end_date: End datetime (inclusive)
        data_path: Directory containing cached data files (default: './cache')

    Returns:
        Polars DataFrame with aggregated OHLC time series for the given range.
    """
    return load_timeseries_range(sym, time_interval, start_date, end_date, OHLC_AGGS, data_path)


def sharpe_annualization_factor(interval: str,
                                trading_days_per_year: int = 252,  # Stocks: ~252 trading days
                                trading_hours_per_day: float = 6.5) -> float:  # Stocks: 9:30-4:00 ET
    """
    Compute annualization factor for stock market given a return interval.
    
    interval : str
        Frequency string like '1d', '1h', '30m', '15s'.
    trading_days_per_year : int
        Number of trading days in a year (default 252 for stocks).
    trading_hours_per_day : float
        Number of trading hours in a trading day (default 6.5 for US stocks).
    ...
    # periods per year - adjusted for stock market hours
    if unit == 'd':
...
def ohlc_timeseries(df: pl.DataFrame, time_interval: str) -> pl.DataFrame:
    """
    For yfinance data, this is mostly a pass-through since data is already OHLC.
    Can be used for resampling if needed.
    ...
    """
    # If data is already OHLC (from yfinance), return as-is
    if all(col in df.columns for col in ['open', 'high', 'low', 'close']):
        return df
    
    # Otherwise use original aggregation (for trade-level data)
...
def add_tx_fee(trades: pl.DataFrame, tx_fee: float, name: str):
    """
    Add transaction fees for stock trading.
    For stocks, fees are typically per-share or flat rather than percentage-based.
    This maintains the interface but adapts for equity markets.
    """
    # For stocks, you might want to use per-share commissions instead
    # Keeping percentage-based for consistency, but could be modified
...
def add_tx_fees(trades: pl.DataFrame, maker_fee: float, taker_fee: float):
    """
    Add maker/taker fees for stock trading.
    Note: Stock commissions work differently than crypto - this maintains interface
    but consider using per-share commissions for more accurate stock modeling.
    """
...
def add_tx_fees_log(trades: pl.DataFrame, maker_fee, taker_fee):
    """
    Add transaction fees in log space for stock trading.
    Maintains interface but adapted for equity market conventions.
    """
